...
2. Read the triage output and classify:
   ...
   - detected WordPress site root and candidate project roots,
   - tooling available (PHP/Composer, Node, @wordpress/scripts),
   - static analysis and linting entrypoints (PHPStan, PHPCS),
...
- If triage reports a nested install (`signals.nestedInstall: true`), scope all commands to the selected plugin/theme target before editing.
